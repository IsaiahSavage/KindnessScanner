...
function ks_da_card_add($ks_db, $creation_location, int $timestamp, int $user_id) {
	return $ks_db->transaction(function($ks_db) {
		$card_id = $ks_db->query_next($ks_db->query('INSERT INTO card (creation_latitude, creation_longitude, creation_time) VALUES ($1, $2, to_timestamp($3)) RETURNING card_id', array($creation_location->getLat(), $creation_location->getLon(), $timestamp)))[0];
...
function ks_da_card_get($ks_db, int $card_id) {
	$row = $ks_db->query_next($ks_db->query('SELECT (card_id, creation_latitude, creation_longitude, extract(epoch FROM creation_time), user_id) FROM card LEFT JOIN r_u_card ON card.card_id = r_u_card.card_id WHERE card_id = $1', array($card_id)));
	if($row) {
		return array(
			'id' => $row[0],
